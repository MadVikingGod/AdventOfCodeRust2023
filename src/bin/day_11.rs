...
fn part1(input: Img) -> i32 {
    part2(input, 2) as i32
}

fn part2(input: Img, scale: i64) -> i64 {
    let xlate = Translation::new(&input);

    input
        .keys()
        .map(|p| xlate.expand(p, scale))
        .collect::<Vec<_>>()
        .iter()
        .tuple_combinations()
        .map(|(p1, p2)| p1 - p2)
...
impl Translation {
    fn new(input: &Img) -> Translation {
        ...
        Translation { x: no_x, y: no_y }
    }

    fn expand(&self, p: &Point<i32>, scale: i64) -> Point<i64> {
